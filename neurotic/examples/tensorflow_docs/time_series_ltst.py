...
def main():
    ...
    column_indices = {k: i for i, k in enumerate(df_train.columns)}
...
def compile_and_fit(model, ds_train, ds_val, epochs=1, patience=2):
...
def plot(
    ...
    for n in range(max_n):
        ...
        plt.scatter(label_indices, labels[n, :, label_col_index], edgecolors='k', label='Labels', c='#2ca02c', s=64)

        if model is not None:
            predictions = model(inputs)
            plt.scatter(label_indices, predictions[n, :, label_col_index], marker='X', edgecolors='k', label='Predictions', c='#ff7f0e', s=64)
